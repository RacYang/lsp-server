...
非治理约束但会改变协议、房间引擎、计时器、恢复、部署或可观测性边界的变更，也应记录独立 ADR。

- Phase 5 示例包括 proto baseline 重置、room 引擎与结算边界、时钟/托管调度、可观测指标最小集合、规则深化与运行时参数。
- Phase 6 示例包括部署形态与 SLO、压测剧本与容量基线、PostgreSQL 备份恢复、密钥与凭据管理、签名提交升级评估。
