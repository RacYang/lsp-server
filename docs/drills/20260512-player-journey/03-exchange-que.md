...
- 协议字段：`Envelope_InitialDeal` / `Envelope_Action(action=exchange_three)` / `Envelope_OpeningActionResp` / `Envelope_OpeningDone`
...
| `[E2.2]` | MUST | 服务端拒绝必须落到 UXTransient 并保留 Marked 让玩家改选 | 旧 `Envelope_ExchangeThreeResp` 只走 `appendResponseLog`，UXNotice 不触发 | reducer 在 `OpeningActionResp` 非 OK 时写 `UXNotice="换三张被拒绝：<reason>"` + 3s TTL；不清 Marked | pass | `A14` / `TestPlayerJourney_E2_2_ExchangeThreeRejectionSurfacesNotice` |
...
| `[E3.2]` | MUST | `OpeningDoneNotify(action=exchange_three)` 后下一帧手牌按权威投影替换 | 既有 `TestApplyExchangeDone*` 已锁 reducer 路径 | 既有 `TestApplyExchangeDoneSwapsThreeTiles` + `TestApplyExchangeDoneUsesPendingAwayWhenProjectionOmitsAway` 保留为锚点 | pass | 既有用例 |
| `[Q1.1]` | MUST | 仅 m/p/s 三键提交定缺；其它键忽略且无副作用 | `1/2/3` 曾下发旧定缺专属请求；`m/p/s` 在非 que_men 阶段会弹「当前不能定缺」UXTransient | 删 `1/2/3` 分支；`m/p/s` 在不在白名单时直接 `return tableEventResult{}`，不写 notice | pass | `A15` / `TestPlayerJourney_Q1_1_QueMenKeysOnlyMPS` + `TestPlayerJourney_Q1_1_QueMenKeysSilentWhenNotAllowed` |
| `[Q1.2]` | MUST | `OpeningDoneNotify(action=que_men)` 后 `view.QueBySeat` 全桌可见 | reducer 已落地，缺独立回归 | `TestPlayerJourney_Q1_2_OpeningMissingSuitDoneFillsRoster` 投递四家 que_suit 后断言 view.QueBySeat 完整 | pass | `A15` / `TestPlayerJourney_Q1_2_OpeningMissingSuitDoneFillsRoster` |
...
- 根因（A14）：换三张交互长期没有把「同花色」与「N/3 计数」当作 cli 层的硬约束，依赖服务端兜底；UXTransient 通道曾在旧专属响应路径上漏接，当前统一由 `OpeningActionResp` 承载。
...
- [x] A14 / 换三张 UI 同花色 + 文案 + 通知：
  ...
  - `cmd/cli/state_apply.go` `Envelope_OpeningActionResp` 在 error_code 非 OK 时把 `envelopeError` 结果写入 `v.UXNotice = "换三张被拒绝：<reason>"`，TTL 3s；不清 cursor.Marked。
- [x] A15 / 定缺键位 + 文案：
  ...
  - `cmd/cli/state_apply.go` `Envelope_OpeningActionResp` 在定缺阶段 error_code 非 OK 时写 `v.UXNotice = "定缺被拒绝：<reason>"`，TTL 3s。
...
- 关键测试名（新增）：
  ...
  - `TestPlayerJourney_Q1_2_OpeningMissingSuitDoneFillsRoster`
