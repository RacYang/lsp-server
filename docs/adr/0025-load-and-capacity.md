...
已采纳。首轮压测剧本、输出格式与容量回写路径已经落地；预发执行编号为 `phase6-preprod-20260428`，详见 `docs/baselines/phase6-preprod-20260428/summary.md`。线上容量基线继续通过 follow-up 修订。
...
每个 Scenario 都给出"通过条件"，把通过条件写到 `bench/scenarios/scenario_*/config.yaml` 中的 YAML 剧本里。

#### 1.2 工具与脚本

- 客户端：基于 `nhooyr.io/websocket` 写最小压测客户端（Go），按 Scenario 加载牌谱与动作脚本。
- 调度：从一台单独的"打手机"运行；不在被测服务自身节点上跑压测客户端，避免自相耗资源。
- 数据仓：把 Prometheus 在压测窗口的 metric 快照导出到 `bench/runs/<run_id>/metrics.json`，便于离线对比；需要长期归档时再人工挪到 `docs/baselines/<run_id>/`。
...
`bench/runs/` 整个目录不入 Git；需要长期引用的执行结果再整理后放入 `docs/baselines/<run_id>/`。
