...
- 但 engine 在 `applyExchangeThree`、`ApplyDiscard`、`drawForCurrentTurn` 等位置改完阶段状态后立即 `progress := rs.roundProgress()` 取 `rs.deadlineUnixMs`，并把它编码进 `ExchangeThreeDoneNotify`、`QueMenDoneNotify`、`ActionNotify`、`DrawTileNotify`、`ClaimPromptNotify` 等多类对外消息。
...
2. **`enterPhase(reason)` 是 phaseReason 与 phaseStartUnixMs 的唯一写入入口**。engine 任何路径切换 `waitingExchange / waitingQueMen / claimWindowOpen / waitingTsumo / waitingDiscard` 等等待态时，必须经 `rs.enterPhase(reason)` 完成；该 helper 内部用注入的 `clock.Clock` 设置 `phaseStartUnixMs = clk.Now().UnixMilli()`。enforcer 在 `make verify-fast` 中静态校验。
...
5. **协议层引入 `PhaseUpdate` 嵌入消息作为客户端唯一的 phase/deadline 接收口**。`PhaseUpdate { Phase phase; int64 step; WaitingReason reason; int64 deadline_unix_ms; int64 server_now_unix_ms; repeated string available_actions; int32 acting_seat }` 必须嵌入所有 `*Notify` 与所有 `*Response`（含拒绝路径）。客户端 reducer 删除散点的 `v.DeadlineUnixMS = ...` 写入，合并为单一 `applyPhaseUpdate(pu)`，所有 envelope 进 reducer 第一步即调它。
6. **客户端动作请求必须绑定 `PhaseToken{ step, reason }`**。所有可触发阶段切换的请求（`ExchangeThreeRequest / QueMenRequest / DiscardRequest / PongRequest / GangRequest / HuRequest / PassRequest / ClaimRequest`）携带 `PhaseToken`。actor 入口校验：`token.step != rs.step || token.reason != rs.phaseReason` 时返回新的错误码 `PHASE_DRIFTED` 并附完整 `PhaseUpdate`；客户端弹"操作已被自动接管"并刷新 UI。
...
- `tile not in hand` 这类家族化错误在 actor 入口层就被 `PHASE_DRIFTED` 替代，错误码语义清晰、可观测，CLI 与 bot 共用同一兜底路径。
