...
玩家期望："别人打的牌我能吃/碰/杠/胡就立刻看到浮窗，倒计时清楚；选一个或过掉。"
...
- `[C3.1] (MUST)` 键位：`←→` 切按钮、`Enter` 确认；快捷键 `h / g / p / c / n` 直选胡 / 杠 / 碰 / 吃 / 过。四川血战默认不开放吃，但若规则候选包含 `chi`，前端必须按同一动作窗口处理。
...
- `[C5.2] (MUST)` 抢杠胡只针对补杠 `bu_gang`，按 ADR-0039 +1 番并取消该补杠事实；不得抢暗杠或直杠。cli 只读结算，不参与计算。
