...
| A15 | P1   | `[Q1.1]`/`[Q2.1]`             | 定缺接受 1/2/3 数字快捷键并对非 que_men 阶段的 m/p/s 产生 UXTransient 副作用；提示文案未告知「选定后不可更改」。**状态：已修复**（`TestPlayerJourney_Q1_1_QueMenKeysOnlyMPS` + `TestPlayerJourney_Q1_1_QueMenKeysSilentWhenNotAllowed` + `TestPlayerJourney_Q1_2_QueMenDoneFillsRoster`） |
